...
class Sub_category_model extends CI_Model
{
	function __construct()
	{
		...
		$this->load->database();
		$this->load->library('sub_category_obj');					
	...
	{					
		...
		$selectStmt 	= " SELECT * ";
		$fromTable		= " FROM " . $this->config->item('tbl_sub_category','dbtables');		
	    ...
	    if ($categoryId != null ){ 
	    	$queryStmt 		.=  " AND category_id = ? "; 	
			$queryDataArr[]	= $categoryId; 				    	
		}	    
	    
		// Add Search condition if any
		if ($searchField != "" && $searchText != ""){
			// Query Example: SELECT id, name, category_id, STATUS FROM mp_sub_category WHERE category_id =0 AND name LIKE 'buy%' 			
	 ...
	 */
	function getRecords($subCategoryId = null, $categoryId = null, $status = null)	
	{ 				
		// Prepare Query - using Query Binding concept from CodeIgnitor
		$queryDataArr 	= array(); //used for data binding
		...
		if ($subCategoryId != null){// note: for Add operation, id=-1 which is correct condition.
			$queryStmt 		.=  " AND id = ? "; 
	    ...
	    if( $queryResult->num_rows() > 0 ){
		   ...
		   if ($subCategoryId != null && count($dataObjArr)>0 ){
		   	return $dataObjArr[0];
		   }
	...
	// This returns the name of sub_category using id	
	function getName($id){
		$qry = "SELECT id, name FROM " . $this->config->item('tbl_sub_category','dbtables') . " WHERE id=? ";
	...
	function getTotalRecords(){		
		return $this->db->count_all($this->config->item('tbl_sub_category','dbtables'));
	}
	
	
	//check for duplicate name for add and edit operations. $argName is used for "add", $argOldName for "edit"
	function isNamePresent($argName, $argOldName="", $categoryId)
	{		
		/* SELECT * FROM `gz_sub_category` where `name` = "newname" and name != 'oldname' */		
		$con = "";
		
		// add categoryId condition
		$con .= " category_id = {$categoryId}   AND ";
		...
		$qry = "SELECT id, name FROM " . $this->config->item('tbl_sub_category','dbtables') . " WHERE " . $con;				
		$queryResult = $this->db->query($qry, $dataBindArr);		
	    $flag = false;
 		if ($queryResult->num_rows() > 0){
	    	$flag = true;			
	...
	{			
		...
		$data = array( // field => value pair - other than primary key
				   ...
				   'category_id' => $argRecordObj->getCategoryId(),
		...
		if ($argRecordObj->getId() == NULL){ // Add operation
			...
			$queryResult = $this->db->insert($this->config->item('tbl_sub_category','dbtables'), $data);			  						
		
		} else if ($argRecordObj->getId() > 0){ // Edit Operation
			
			$this->db->where('id', $argRecordObj->getId());
			$queryResult = $this->db->update($this->config->item('tbl_sub_category','dbtables'), $data);
	...
	function delete($argIds){
		$queryResult = $this->db->query( "DELETE FROM " . $this->config->item('tbl_sub_category','dbtables') . " WHERE id IN (" . $argIds . ")" );
		return $queryResult;
	}
	
	function getEntityObject($row){
		
	    $dataObj = new $this->sub_category_obj;	 		
   	
		$dataObj->setId($row->id);
		$dataObj->setName($row->name);
		$dataObj->setCategoryId($row->category_id);		
