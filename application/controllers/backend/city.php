...
class City extends CI_Controller {
	...
	{					
		// Get data for query
		$listDataObj	= $this->city_list->getInstance($argData);
		
		$dataArr 		= $this->city_model->getList($listDataObj);
		// set values	
		$listDataObj->setDbDataArr($dataArr['dbDataArr']);
		$listDataObj->setTotalRecords($dataArr['totalRecords']);
		
		// Set Pagination
		$pageDataArr = $this->city_list->getPaginationArray();	
	...
	{		
		...
		$listDataObj		= $this->city_list->getInstance();	
		...
		$viewArr['listDataObj']		= $listDataObj;
 		...
 		// Display output page		
		$viewArr['viewPage'] 	= "city_add_edit";
		$this->load->view('backend/layout', $viewArr);
	...
	function save()
	{
		...
		// Create city object based on user input
		...
		{				
			...
			$queryFlag = $this->city_model->save($recordObj);
	...
	/**
	 * This function creates city object having data provided by user.
	 */	
	function createRecordObj(){
		// Create city object, set values
		$recordObj = new $this->city_obj;		
	...
	{	
		...
		if (!empty($ids)){
			// Execute query
			$queryFlag = $this->city_model->delete($ids);
		...
		// Get data object
		$listDataObj		= $this->city_list->getInstance($dataArr);				
	...
	function setFormValidationRules($argViewArr){
		...
		// For Edit operation, user is allowed to change City-name. Check if name is changed, if yes, perform unique validation
		if( $argViewArr['oprType'] == OPR_EDIT && (trim($this->input->post('name')) == $this->input->post('oldName')) ){
			...
			$this->form_validation->set_rules('name', 'City Name', 'trim|required|min_length['.NAME_MIN_LENGTH.']|max_length['.NAME_MAX_LENGTH.']|xss_clean');	
		}else{
			// for Add operation OR name changed, check for uniqueness
			$this->form_validation->set_rules('name', 'City Name', 'trim|required|min_length['.NAME_MIN_LENGTH.']|max_length['.NAME_MAX_LENGTH.']|xss_clean|callback_isUniqueName[name]');	
		}		
	...
	/**
	 * 
	 *This function checks for unique city name
	 * @param $argName
	 */
	function isUniqueName($argName){
		
		// Check whether name is present or not by calling method from Model		
		$presentFlag = $this->city_model->isNamePresent($argName);
		
		// Check the flag
		if ($presentFlag == true){
			// Name found in database
			$this->form_validation->set_message('isUniqueName', $this->lang->line('info.duplicate.city'));
			return false;			
		}else{
	...
	function getList(){
		//get list of areas 
		$cityId = null;	
		$stateId = $this->input->post('stateId'); // Note that this parameter is sent by AJAX call
		
		$dataArr = $this->city_model->getRecords($cityId, $stateId, ACTIVE); 
