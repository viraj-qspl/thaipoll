...
class Category extends CI_Controller {
	...
	{	
        // Write to log file
 		log_message('info', '--------------> Category - Index() called');		
	...
	{					
        // Write to log file
 		log_message('info', '--------------> Category - listing() called');		
		
		// Get data for query
		$listDataObj	= $this->category_list->getInstance($argData);			
		$dataArr 		= $this->category_model->getList($listDataObj);
//var_dump($listDataObj);
	
		log_message('info', '--------------> $listDataObj id ' . $listDataObj->getId() );	
		...
		// Set Pagination
		$pageDataArr = $this->category_list->getPaginationArray();	
	...
	{		
        // Write to log file
 		log_message('info', '--------------> Category - search() called');		
	...
	{		
        // Write to log file
 		log_message('info', '--------------> Category - addEditForm() called');		
		
		// Get data object and execute query
		$listDataObj		= $this->category_list->getInstance();	
		...
		$viewArr['listDataObj']		= $listDataObj;
 		...
 		// Display output page		
		$viewArr['viewPage'] 	= "category_add_edit";
		$this->load->view('backend/layout', $viewArr);
	...
	{
        // Write to log file
 		log_message('info', '--------------> Category - save() called');		
		...
		{				
			...
			$queryFlag = $this->category_model->save($recordObj);
	...
	function createRecordObj(){
        // Write to log file
 		log_message('info', '--------------> Category - createRecordObj() called');		
	...
	{	
        // Write to log file
 		log_message('info', '--------------> Category - delete() called');		
		...
		// Get data object
		$listDataObj		= $this->category_list->getInstance($dataArr);				
	...
	function setFormValidationRules($argViewArr){
        // Write to log file
 		log_message('info', '--------------> Category - setFormValidationRules() called');		
		...
		if( $argViewArr['oprType'] == OPR_EDIT && (trim($this->input->post('name')) == $this->input->post('oldName')) ){
			...
			$this->form_validation->set_rules('name', 'Category Name', 'trim|required|min_length['.NAME_MIN_LENGTH.']|max_length['.NAME_MAX_LENGTH.']|xss_clean');	
		}else{
			// for Add operation OR name changed, check for uniqueness
			$this->form_validation->set_rules('name', 'Category Name', 'trim|required|min_length['.NAME_MIN_LENGTH.']|max_length['.NAME_MAX_LENGTH.']|xss_clean|callback_isUniqueName[name]');	
		}		
	...
	function isUniqueName($argName){
        // Write to log file
 		log_message('info', '--------------> Category - isUniqueName() called');		
		
		// Check whether name is present or not by calling method from Model		
		$presentFlag = $this->category_model->isNamePresent($argName);
		
		// Check the flag
		if ($presentFlag == true){
			// Name found in database
			$this->form_validation->set_message('isUniqueName', $this->lang->line('info.duplicate.category'));
			return false;			
		}else{
	...
	function getList(){
        // Write to log file
 		log_message('info', '--------------> Category - getList() called');		
		
		$categoryId = null;

		//get list of subcategories 
		$categoryArr = $this->category_model->getRecords($categoryId, ACTIVE);
