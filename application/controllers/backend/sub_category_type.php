...
class Sub_category_type extends CI_Controller {
    ...
    {
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - construct() called');		
    	
    	// Call to Controller constructor
        parent::__construct();
            		
        // Load helpers, models and library - autoload.php has defined common loads
		$this->load->library(array('sub_category_type_obj', 'sub_category_type_list'));
		$this->load->model('sub_category_type_model');	
	...
	{	
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - index() called');		
	...
	{		
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - listing() called');		
		
		// Get data for query
		$listDataObj	= $this->sub_category_type_list->getInstance($argData);
		$dataArr 		= $this->sub_category_type_model->getList($listDataObj);
		...
		// Set Pagination
		$pageDataArr = $this->sub_category_type_list->getPaginationArray();	
		...
		// Display output page		
		$viewArr['viewPage'] 	= "sub_category_type";
	...
	{		
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - search() called');		
	...
	{		
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - addEditForm() called');		
		
		// Get data object and execute query
		$listDataObj		= $this->sub_category_type_list->getInstance();	
		...
		if (is_object($argRecordObj) && $argRecordObj != NULL ){
			...
			$recordObj	= $this->sub_category_type_model->getRecords($listDataObj->getId());
		}	

		// Based on recordObj set other parameters		
		if ($recordObj == NULL){ // Add operation
			$dataArr['oprType'] = OPR_ADD;
			$recordObj = new $this->sub_category_type_obj;					 					
		}else { // Edit operation	
		...
		$viewArr['listDataObj']		= $listDataObj;
 		...
 		// Display output page		
		$viewArr['viewPage'] 	= "sub_category_type_add_edit";
		$this->load->view('backend/layout', $viewArr);
	...
	{
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - save() called');		
		...
		{				
			...
			$queryFlag = $this->sub_category_type_model->save($recordObj);
	...
	function createRecordObj(){
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - createRecordObj() called');		
		...
		//note: CategoryId is not stored in subcatType table (due to normalisation concept)
		$recordObj->setSubCategoryId($this->input->post('subCategoryId'));		
	...
	{	
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - delete() called');		
		...
		if (!empty($ids)){
			// Execute query
			$queryFlag = $this->sub_category_type_model->delete($ids);
		...
		// Get data object
		$listDataObj		= $this->sub_category_type_list->getInstance($dataArr);				
	...
	function setFormValidationRules($argViewArr){
		...
		if( $argViewArr['oprType'] == OPR_EDIT && (trim($this->input->post('name')) == $this->input->post('oldName')) ){
			...
			$this->form_validation->set_rules('name', 'Subcategory Type Name', 'trim|required|min_length['.NAME_MIN_LENGTH.']|max_length['.NAME_MAX_LENGTH.']|xss_clean');	
		}else{
			// for Add operation OR name changed, check for uniqueness
			$this->form_validation->set_rules('name', 'Subcategory Type Name', 'trim|required|min_length['.NAME_MIN_LENGTH.']|max_length['.NAME_MAX_LENGTH.']|xss_clean|callback_isUniqueName[name]');	
		}		
	...
	function isUniqueName($argName){
		...
		$presentFlag = $this->sub_category_type_model->isNamePresent($argName, "", $subCategoryId);
	...
	function getTypesForSubcategory(){
        // Write to log file
 		log_message('info', '--------------> Sub_category_type - getTypesForSubcategory() called');				
		 		
		$subCategoryId 		= $this->input->post("subCategoryId");

		//get list of subcategories 
		$subCategoryTypeArr = $this->sub_category_type_model->getTypesForSubcategory($subCategoryId, ACTIVE);
		echo json_encode($subCategoryTypeArr);
		exit;
	}
