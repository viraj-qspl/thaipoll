...
class State extends CI_Controller {
	...
	{					
		// Get data for query
		$listDataObj	= $this->state_list->getInstance($argData);
		
		$dataArr 		= $this->state_model->getList($listDataObj);
		// set values	
		$listDataObj->setDbDataArr($dataArr['dbDataArr']);
		$listDataObj->setTotalRecords($dataArr['totalRecords']);
		
		// Set Pagination
		$pageDataArr = $this->state_list->getPaginationArray();	
	...
	{		
		...
		$listDataObj		= $this->state_list->getInstance();	
		...
		$viewArr['listDataObj']		= $listDataObj;
 		...
 		// Display output page		
		$viewArr['viewPage'] 	= "state_add_edit";
		$this->load->view('backend/layout', $viewArr);
	...
	function save()
	{
		...
		{				
			...
			$queryFlag = $this->state_model->save($recordObj);
	...
	{	
		...
		if (!empty($ids)){
			// Execute query
			$queryFlag = $this->state_model->delete($ids);
		...
		// Get data object
		$listDataObj		= $this->state_list->getInstance($dataArr);				
	...
	function setFormValidationRules($argViewArr){
		...
		if( $argViewArr['oprType'] == OPR_EDIT && (trim($this->input->post('name')) == $this->input->post('oldName')) ){
			...
			$this->form_validation->set_rules('name', 'State Name', 'trim|required|min_length['.NAME_MIN_LENGTH.']|max_length['.NAME_MAX_LENGTH.']|xss_clean');	
		}else{
			// for Add operation OR name changed, check for uniqueness
			$this->form_validation->set_rules('name', 'State Name', 'trim|required|min_length['.NAME_MIN_LENGTH.']|max_length['.NAME_MAX_LENGTH.']|xss_clean|callback_isUniqueName[name]');	
		}		
	...
	function isUniqueName($argName){
		...
		$presentFlag = $this->state_model->isNamePresent($argName, "", $countryId);
		
		// Check the flag
		if ($presentFlag == true){
			// Name found in database
			$this->form_validation->set_message('isUniqueName', $this->lang->line('info.duplicate.state'));
			return false;			
		}else{
	...
	function getList(){
		...
		$stateId 	= null;
		$countryId 	= $this->input->post('countryId'); //Note: this parameter is sent by various Ajax call.
	
		$subCategoryArr = $this->state_model->getRecords(null, $countryId, ACTIVE);
