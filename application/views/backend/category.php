					...
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						...
						<tr><td colspan="5"> 
							<table width="100%" border="0"><tr>
								...
								|  <a href="<?php echo $this->config->item('categoryAddBeAction')."/".$listDataObj->getUrlParams(PARAM_ADD); ?>"> <?php echo $this->lang->line('link.add'); ?></a> 
								</td>															
								<td width="50%" align="right"> 						
									<form name="searchForm" action="<?php echo $this->config->item('categorySearchBeAction')."/".$listDataObj->getUrlParams(); ?>" method="post">								
						...
						foreach ($listArr as $ind => $categoryObj) { 
							
							//Store the id of each record
							$listDataObj->setId($categoryObj->getId());

							$editAction 	= $this->config->item('categoryEditBeAction')."/".$listDataObj->getUrlParams();	
							$deleteAction 	= $this->config->item('categoryDeleteBeAction')."/".$listDataObj->getUrlParams();
							$listingAction	= $this->config->item('subCategoryListingBeAction')."/".$listDataObj->getUrlParams(PARAM_LISTING);
						?>					
											
						<tr class="adm_highlight_row" id="<?php echo $categoryObj->getId(); ?>" onmouseover="this.className = 'highlight_row';" onmouseout="this.className = 'adm_highlight_row';"   >
							<td align="center"> <input type="checkbox" name="rowCheckbox" value="<?php echo $categoryObj->getId(); ?>" > </td>
							<td align="left"><a href="<?php echo $listingAction; ?>"><?php echo $categoryObj->getName(); ?></a></td>
							<td align="left"><?php echo ucfirst($categoryObj->getStatus()); ?></td>							
							<td align="center"> <a href="javascript:void()" onClick="editRecord('<?php echo $categoryObj->getId(); ?>', '<?php echo $editAction ?>')"><img title="Edit" src="<?php echo $this->config->item('editImage'); ?>" /></a> </td>
				        </tr>
	...
	<?php foreach ($listArr as $ind => $categoryObj) { ?>
		nameArray[<?php echo $categoryObj->getId(); ?>] = "<?php echo $categoryObj->getName() ?>" ;
	<?php } ?>
