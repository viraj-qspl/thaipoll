				 	...
				 	<div id="adm_list_head_wrap"  >
						...
						<div style="float:left; width:400" >
							<strong class="form_add_heading3">
								...
								&gt;
								<?php echo $this->lang->line('lable.category.listing.title');  ?>	 													
							</strong>
						</div>
					...
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						...
						<tr><td colspan="5"> 
							<table width="100%" border="0"><tr>
								...
								|  <a href="<?php echo $this->config->item('subCategoryTypeAddBeAction')."/".$listDataObj->getUrlParams(PARAM_ADD); ?>"> <?php echo $this->lang->line('link.add'); ?></a> 
								</td>															
								<td width="50%" align="right"> 						
									<form name="searchForm" action="<?php echo $this->config->item('subCategoryTypeSearchBeAction')."/".$listDataObj->getUrlParams(); ?>" method="post">								
						...
						foreach ($listArr as $ind => $subCategoryTypeObj) { 
							//Store the id of each record
							$listDataObj->setId($subCategoryTypeObj->getId());
																
							$editAction 		= $this->config->item('subCategoryTypeEditBeAction')."/".$listDataObj->getUrlParams();	
							$deleteAction 		= $this->config->item('subCategoryTypeDeleteBeAction')."/".$listDataObj->getUrlParams();
						?>					
											
						<tr class="adm_highlight_row" id="<?php echo $subCategoryTypeObj->getId(); ?>" onmouseover="this.className = 'highlight_row';" onmouseout="this.className = 'adm_highlight_row';"   >
							<td align="center"> <input type="checkbox" name="rowCheckbox" value="<?php echo $subCategoryTypeObj->getId(); ?>" > </td>
							<td align="left"><?php echo $subCategoryTypeObj->getName(); ?></td>
							<td align="left"><?php echo ucfirst($subCategoryTypeObj->getStatus()); ?></td>							
							<td align="center">
								<a href="javascript:void()" onClick="editRecord('<?php echo $subCategoryTypeObj->getId(); ?>', '<?php echo $editAction ?>')"><img title="Edit" src="<?php echo $this->config->item('editImage'); ?>" /></a>								
							</td>
				        </tr>
	...
	<?php foreach ($listArr as $ind => $subCategoryTypeObj) { ?>
		nameArray[<?php echo $subCategoryTypeObj->getId(); ?>] = "<?php echo $subCategoryTypeObj->getName() ?>" ;
	<?php } ?>
