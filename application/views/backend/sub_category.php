					...
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						...
						<tr><td colspan="5"> 
							<table width="100%" border="0"><tr>
								...
								|  <a href="<?php echo $this->config->item('subCategoryAddBeAction')."/".$listDataObj->getUrlParams(PARAM_ADD); ?>"> <?php echo $this->lang->line('link.add'); ?></a> 
								</td>															
								<td width="50%" align="right"> 						
									<form name="searchForm" action="<?php echo $this->config->item('subCategorySearchBeAction')."/".$listDataObj->getUrlParams(); ?>" method="post">								
						...
						$editAction = $deleteAction  = $listingAction = "";
						foreach ($listArr as $ind => $subCategoryObj) { 
							//Store the id of each record
							$listDataObj->setId($subCategoryObj->getId());

																
							$editAction 		= $this->config->item('subCategoryEditBeAction')."/".$listDataObj->getUrlParams();	
							$deleteAction 		= $this->config->item('subCategoryDeleteBeAction')."/".$listDataObj->getUrlParams();
							$listingAction		= $this->config->item('subCategoryTypeListingBeAction')."/".$listDataObj->getUrlParams(PARAM_LISTING);
						?>					
											
						<tr class="adm_highlight_row" id="<?php echo $subCategoryObj->getId(); ?>" onmouseover="this.className = 'highlight_row';" onmouseout="this.className = 'adm_highlight_row';"   >
							<td align="center"> <input type="checkbox" name="rowCheckbox" value="<?php echo $subCategoryObj->getId(); ?>" > </td>
							<td align="left"><a href="<?php echo $listingAction; ?>"><?php echo $subCategoryObj->getName(); ?></a></td>
							<td align="left"><?php echo ucfirst($subCategoryObj->getStatus()); ?></td>							
							<td align="center">
								<a href="javascript:void()" onClick="editRecord('<?php echo $subCategoryObj->getId(); ?>', '<?php echo $editAction ?>')"><img title="Edit" src="<?php echo $this->config->item('editImage'); ?>" /></a>								
							</td>
				        </tr>
	...
	<?php foreach ($listArr as $ind => $subCategoryObj) { ?>
		nameArray[<?php echo $subCategoryObj->getId(); ?>] = "<?php echo $subCategoryObj->getName() ?>" ;
	<?php } ?>
