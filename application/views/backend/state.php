					...
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						...
						<tr><td colspan="5"> 
							<table width="100%" border="0"><tr>
								...
								|  <a href="<?php echo $this->config->item('stateAddBeAction')."/".$listDataObj->getUrlParams(PARAM_ADD); ?>"> <?php echo $this->lang->line('link.add'); ?></a> 
								</td>															
								<td width="50%" align="right"> 						
									<form name="searchForm" action="<?php echo $this->config->item('stateSearchBeAction')."/".$listDataObj->getUrlParams(); ?>" method="post">								
						...
						foreach ($listArr as $ind => $stateObj) { 
							//Store the id of each record
							$listDataObj->setId($stateObj->getId());
																
							$editAction 		= $this->config->item('stateEditBeAction')."/".$listDataObj->getUrlParams();	
							$deleteAction 		= $this->config->item('stateDeleteBeAction')."/".$listDataObj->getUrlParams();
							$listingAction		= $this->config->item('cityListingBeAction')."/".$listDataObj->getUrlParams(PARAM_LISTING);
						?>					
											
						<tr class="adm_highlight_row" id="<?php echo $stateObj->getId(); ?>" onmouseover="this.className = 'highlight_row';" onmouseout="this.className = 'adm_highlight_row';"   >
							<td align="center"> <input type="checkbox" name="rowCheckbox" value="<?php echo $stateObj->getId(); ?>" > </td>
							<td align="left"><a href="<?php echo $listingAction; ?>"><?php echo $stateObj->getName(); ?></a></td>
							<td align="left"><?php echo ucfirst($stateObj->getStatus()); ?></td>							
							<td align="center">
								<a href="javascript:void()" onClick="editRecord('<?php echo $stateObj->getId(); ?>', '<?php echo $editAction ?>')"><img title="Edit" src="<?php echo $this->config->item('editImage'); ?>" /></a>								
							</td>
				        </tr>
	...
	<?php foreach ($listArr as $ind => $stateObj) { ?>
		nameArray[<?php echo $stateObj->getId(); ?>] = "<?php echo $stateObj->getName() ?>" ;
	<?php } ?>
