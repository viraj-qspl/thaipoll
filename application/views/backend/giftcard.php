				...
				<tr> 
				 <td> 
				 	<div id="adm_list_head_wrap"  >				 	
						...
						&gt; <?php echo $this->lang->line('lable.list'); ?></strong></div>
				...
				<tr>
				 <td> 
					...
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						...
						<tr>
						   <td colspan="6" ><?php  echo $listDataObj->getOprMessageFormated(); ?></td>
						</tr>		
						...
						<tr><td colspan="6"> 
							<table width="100%" border="0"><tr>
								...
								|  <a href="<?php echo $this->config->item('giftcardAddBeAction')."/".$listDataObj->getUrlParams(PARAM_ADD); ?>"> <?php echo $this->lang->line('link.add'); ?></a> 
								</td>															
								<td width="50%" align="right"> 						
									<form name="searchForm" action="<?php echo $this->config->item('giftcardSearchBeAction')."/".$listDataObj->getUrlParams(); ?>" method="post">								
						...
						<tr class="form_add_heading4">
							...
							<td width="15%" align="left"><?php echo $sortFieldArr["title"]; ?></td>
							<td width="20%" align="left"><?php echo $sortFieldArr["descp"]; ?></td>
							<td width="20%" align="left"><?php echo $sortFieldArr["value"]; ?></td>
							<td width="20%" align="left"><?php echo $sortFieldArr["points"]; ?></td>
							<td width="20%" align="left"><?php echo $sortFieldArr["status"]; ?></td>														
						...
						<!--  Listing data -->							
						<tr><td colspan="7"> <div class="adm_border1" ></div> </td></tr>
						...
						foreach ($listArr as $ind => $giftcardObj) { 
							//Store the id of each record
							$listDataObj->setId($giftcardObj->getId());
																
							$editAction 		= $this->config->item('giftcardEditBeAction')."/".$listDataObj->getUrlParams();	
							$deleteAction 		= $this->config->item('giftcardDeleteBeAction')."/".$listDataObj->getUrlParams();
						?>					
											
						<tr class="adm_highlight_row" id="<?php echo $giftcardObj->getId(); ?>" onmouseover="this.className = 'highlight_row';" onmouseout="this.className = 'adm_highlight_row';"   >
							<td align="center"> <input type="checkbox" name="rowCheckbox" value="<?php echo $giftcardObj->getId(); ?>" > </td>							
							<td align="left"><?php echo $giftcardObj->getTitle(); ?></td>							
							<td align="left"><?php echo $giftcardObj->getDescp(); ?></td>							
							<td align="left"><?php echo $giftcardObj->getValue(); ?></td>							
							<td align="left"><?php echo ucfirst($giftcardObj->getStatus()); ?></td>								
							<td align="left"><?php echo ucfirst($giftcardObj->getPoints()); ?></td>	
											
							<td align="center">
								<a href="javascript:void()" onClick="editRecord('<?php echo $giftcardObj->getId(); ?>', '<?php echo $editAction ?>')"><img title="Edit" src="<?php echo $this->config->item('editImage'); ?>" /></a>								
							</td>
				        </tr>
	...
	<?php foreach ($listArr as $ind => $giftcardObj) { ?>
		nameArray[<?php echo $giftcardObj->getId(); ?>] = "<?php echo $giftcardObj->getTitle(); ?>";
	<?php } ?>
