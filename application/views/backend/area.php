					...
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						...
						<tr><td colspan="5"> 
							<table width="100%" border="0"><tr>
								...
								|  <a href="<?php echo $this->config->item('areaAddBeAction')."/".$listDataObj->getUrlParams(PARAM_ADD); ?>"> <?php echo $this->lang->line('link.add'); ?></a> 
								</td>															
								<td width="50%" align="right"> 						
									<form name="searchForm" action="<?php echo $this->config->item('areaSearchBeAction')."/".$listDataObj->getUrlParams(); ?>" method="post">								
						...
						foreach ($listArr as $ind => $areaObj) { 
							//Store the id of each record
							$listDataObj->setId($areaObj->getId());

							$editAction 		= $this->config->item('areaEditBeAction')."/".$listDataObj->getUrlParams();	
							$deleteAction 		= $this->config->item('areaDeleteBeAction')."/".$listDataObj->getUrlParams();
						?>					
											
						<tr class="adm_highlight_row" id="<?php echo $areaObj->getId(); ?>" onmouseover="this.className = 'highlight_row';" onmouseout="this.className = 'adm_highlight_row';"   >
							<td align="center"> <input type="checkbox" name="rowCheckbox" value="<?php echo $areaObj->getId(); ?>" > </td>
							<td align="left"><?php echo $areaObj->getName(); ?></td>
							<td align="left"><?php echo ucfirst($areaObj->getStatus()); ?></td>							
							<td align="center"></td>							
							<td align="center">
								<a href="javascript:void()" onClick="editRecord('<?php echo $areaObj->getId(); ?>', '<?php echo $editAction ?>')"><img title="Edit" src="<?php echo $this->config->item('editImage'); ?>" /></a>								
							</td>
				        </tr>
	...
	<?php foreach ($listArr as $ind => $areaObj) { ?>
		nameArray[<?php echo $areaObj->getId(); ?>] = "<?php echo $areaObj->getName() ?>" ;
	<?php } ?>
