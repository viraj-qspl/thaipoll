					...
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						...
						<tr><td colspan="5"> 
							<table width="100%" border="0"><tr>
								...
								|  <a href="<?php echo $this->config->item('userAddBeAction')."/".$listDataObj->getUrlParams(PARAM_ADD); ?>"> <?php echo $this->lang->line('link.add'); ?></a> 
								</td>															
								<td width="50%" align="right"> 						
									<form name="searchForm" action="<?php echo $this->config->item('userSearchBeAction')."/".$listDataObj->getUrlParams(); ?>" method="post">								
						...
						<tr class="form_add_heading4">
							...
							<td width="40%" align="left"><?php echo $sortFieldArr["first_name"]; ?></td>
						...
						foreach ($listArr as $ind => $userObj) { 
							//Store the id of each record
							$listDataObj->setId($userObj->getId());
																
							$editAction 		= $this->config->item('userEditBeAction')."/".$listDataObj->getUrlParams();	
							$deleteAction 		= $this->config->item('userDeleteBeAction')."/".$listDataObj->getUrlParams();
						?>					
											
						<tr class="adm_highlight_row" id="<?php echo $userObj->getId(); ?>" onmouseover="this.className = 'highlight_row';" onmouseout="this.className = 'adm_highlight_row';"   >
							<td align="center"> <input type="checkbox" name="rowCheckbox" value="<?php echo $userObj->getId(); ?>" > </td>
							<td align="left"><?php echo $userObj->getFirstName()." ".$userObj->getLastName(); ?></td>
							<td align="left"><?php echo ucfirst($userObj->getStatus()); ?></td>							
							<td align="center"></td>							
							<td align="center">
								<a href="javascript:void()" onClick="editRecord('<?php echo $userObj->getId(); ?>', '<?php echo $editAction ?>')"><img title="Edit" src="<?php echo $this->config->item('editImage'); ?>" /></a>								
							</td>
				        </tr>
	...
	<?php foreach ($listArr as $ind => $userObj) { ?>
		nameArray[<?php echo $userObj->getId(); ?>] = "<?php echo $userObj->getFirstName()." ".$userObj->getLastName(); ?>";
	<?php } ?>
